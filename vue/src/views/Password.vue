...
const save = () => {
  if (state.form.newPassword !== state.form.confirmPassword) {
    ElMessage.warning('The new password entered twice does not match')
    return
  }
  if (state.form.newPassword === state.form.password) {
    ElMessage.warning('The old and new passwords cannot be same')
    return
  }
  request.post("/password/change", state.form).then(res => {
    if (res.code === '200') {
      ElMessage.success('The password was changed successfully')
      userStore.logout()
    } else {
  ...
  <div>
    <el-card style="width: 40%; margin: 10px auto">
      <el-form style="width: 80%; margin: 0 auto" label-width="110px" :model="state.form" :rules="rules" ref="ruleFormRef">
        <el-form-item label="enterTheOriginalPassword" prop="password">
          <el-input show-password v-model="state.form.password"></el-input>
        </el-form-item>
        <el-form-item label="enterTheNewPassword" prop="newPassword">
          <el-input show-password v-model="state.form.newPassword"></el-input>
        </el-form-item>
        <el-form-item label="enterTheNewPassword" prop="confirmPassword">
